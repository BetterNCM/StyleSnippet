...
function Menu() {
    ...
    async function reCompileStylesheets(snippets: StylesheetSnippet[]) {
        return await Promise.all(
            snippets.map(async (snippet) => {
                return `/* StyleSnippet ${snippet.info.id
    ...
    const checkEditorOpen = async () => {
        ...
        if (!editorWin || editorWin.closed) {
            ...
            evt.addEventListener("savefile", (evt: CustomEvent) => {
                const info: SnippetInfo = evt.detail.info;
                betterncm_native.fs.writeFileText(info.file, evt.detail.stylesheet);
                calcStylesheets();
            })
        ...
        <div className="stylesnippet-config">
            ...
            {onlineSnippetList.filter(snippet => !snippet.hidden).map((snippet) => (
                <div
                    ...
                    />
                    <span className="snippet-name" onClick={() => toggleSnippet(snippet.id)}>{snippet.name}</span>
